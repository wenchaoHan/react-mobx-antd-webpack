    ...
    return function (Component) {
        return formWrapper((0, _createReactClass2["default"])({
            ...
            render: function render() {
                ...
                var vf = this.props.form.validateFields;
                if (!options || !options.hidePopMessage) {
                    this.props.form.validateFields = function (ns, opt, cb) {
                        var params = getParams(ns, opt, cb);
                        return vf(params.names, params.options, function (err) {
                            for (var _len2 = arguments.length, args = Array(_len2 > 1 ? _len2 - 1 : 0), _key2 = 1; _key2 < _len2; _key2++) {
                                args[_key2 - 1] = arguments[_key2];
                            }

                            if (err && Object.keys(err).length) {
                                var msg = '对不起，您提交的信息存在错误，请修改后再重新提交';
                                if (options && typeof options.getErrorMessage === 'function') {
                                    msg = options.getErrorMessage(err);
                                }
                                _message2["default"].error(msg, 2.5);
                                if (!options || !options.errorPass) {
                                    return;
                                }
                            }
                            if (typeof params.callback === 'function') {
                                params.callback.apply(params, [err].concat(args));
                            }
                        });
                    };
                }
